 ...
 *
 * It supports data providers, which allow to merge new items with existing ones
 * before saving. Data providers are executed in order and each one replaces the
 * data merged by the previous provider.
 *
 * @param {String} dataFile Bundle file to write items to. Cannot be null.
 * @param {DataProvider[]} [dataProviders] List of data providers to merge new
 *   items before saving.
 * @constructor
 */
module.exports = function FileSystemStorer(dataFile, dataProviders) {
  ...
  var merge = function (data) {
    return new Promise((resolve, reject) => {
      ...
      async.eachSeries(dataProviders, (dataProvider, next) => {
        dataProvider.merge(item).then(mergedItem => {
          item = mergedItem;
          next();
        }).catch(err => next(err));
      }, err => {
    ...
    store (id, data, role) {
      return new Promise((resolve, reject) => {
        if (dataProviders && dataProviders.length > 0) {
          resolve(merge(data).then(item => writeItem(id, item)));
        } else {
          writeItem(id, data);
          resolve();
        }
