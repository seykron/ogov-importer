...
module.exports = function BillDocumentParser($, document) {
  ...
  var extractBill = function () {
    ...
    if (generalInformation.find("a").length > 0) {
      bill.textUrl = generalInformation.find("a").extractUrl();
    }
    if (generalInformation.length > 10) {
      // Standard format, no additional information.
      bill.summary = generalInformation.eq(11).defaultIfEmpty();
    } else {
  ...
  return {
    ...
    parse () {
      return new Promise((resolve, reject) => {
        var bill = extractBill();

        bill.subscribers = extractSubscribers();
        bill.committees = extractCommittees();
        bill.dictums = extractDictums();
        bill.procedures = extractProcedures();
