...
module.exports = function BillImporter(context, options) {
  ...
  return {
    ...
    /** Bill comparator. It compares by modification date in descending order.
     */
    compare (bill1, bill2) {
    ...
    changed (oldBillGroup, newBill) {
      var sourceDate = Date.parse(oldBillGroup[oldBillGroup.length - 1]
        .creationTime);
      var newDate = Date.parse(newBill.creationTime);

      return sourceDate !== newDate;
    }
  };
};
