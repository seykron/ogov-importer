...
module.exports = function BillImporter(context, options) {
  ...
  /** Start date. */
  const START_DATE = options && ("01/01/" + options.start) || "01/01/1999";

  /** Softpage document format supported by this reader.
   * @constant
   * @private
   */
  const DATA_SOURCE = "http://www1.hcdn.gov.ar/proyectos_search/resultado.asp?" +
    "ordenar=2" +
    ...
    "&fecha_inicio=" + START_DATE + // Bills start date.
  ...
  var getEndDate = function () {
    ...
    if (config.end) {
      if (config.end.indexOf("-") > -1
          || config.end.indexOf("/") > -1) {
        return config.end.replace(/-/ig, "/")
      } else {
        return "31/12/" + config.end;
      }
    } else {
