...
module.exports = function PeopleImporter(importer, options) {
      ...
      importer.load(URL_LEG).then($ => {
        ...
        async.forEachLimit(people, 4, function (person, next) {
          ...
          fetchCommittees({
            ...
            importer.store(person.user, person).nodeify(next);
          }).catch(err => {
  ...
  var extractSenators = function (callback) {
    return new Promise((resolve, reject) => {
      debug("loading senators: %s", URL_LEG);

      importer.load(URL_SEN).then($ => {
        ...
        async.forEachLimit(peopleEl, 4, function (item, next) {
          ...
          person = {
            ...
            email: InputUtils.errorIfEmpty(contactInfo.shift()),
            phone: InputUtils.errorIfEmpty(contactInfo.shift()),
            extension: InputUtils.errorIfEmpty(contactInfo.shift()),
            role: "senator"
          };
  ...
  return {
     ...
     */
    run () {
      return Promise.all(extractSenators(), extractLegislatives());
    }
  };
};
