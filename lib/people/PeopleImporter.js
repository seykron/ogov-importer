...
module.exports = function PeopleImporter(options) {
  ...
  var parseInvalidTable = function (cells) {
    ...
    for (i = 0; i < cells.length; i += 6) {
      userEl = cells.eq(i + 1).find("a");

      if (userEl.length > 0) {
        user = userEl.attr("href").replace(/\/diputados\//, "").replace("/", "");
        ...
        LOG.info("No info for user " + userEl.html());
      }
    }
    return people;
  };
  ...
  return extend(importer, {
    ...
    execute: function (task, callback) {
      var proceed = function (err) {
        ...
        callback(err, pendingTasks === 0);
      };
      if (task.data.url === URL_LEG){
        extractLegislatives(proceed);
      } else {
        extractSenators(proceed);
      }
    },
